...
results_df = pd.DataFrame(
    columns=['simulation', 'num_env', 'num_phen', 'mean_duration', 'total_time', 'mumax', 'mean_mu_max', 'delta',
...
if not READ_IN_DATA:
    ...
    num_envs = np.unique(np.floor(np.exp(np.linspace(np.log(2), np.log(20), 10))).astype(int))
    num_phen_factors = np.unique(np.floor(np.linspace(1, 2, 2)).astype(int))
    mean_durations = np.linspace(5, 200, 10)
    mu_maxs = np.linspace(0.9, 1.2, 1)
    deltas = np.linspace(0.05, .95, 10)
    sim_time_factor = 0.5
    time_lb = 1000
    time_ub = 10000
    ...
    iterables = product(num_envs, num_phen_factors, mean_durations, mu_maxs, deltas)
    n_simus = len(num_envs) * len(num_phen_factors) * len(mean_durations) * len(mu_maxs) * len(deltas)
    simulation = -1
    ...
    for iterable in iterables:
        ...
        num_env, num_phen_factor, mean_duration, mumax, delta = iterable
        ...
        for ind_env in range(num_env):
            bad_mus = np.random.uniform(0, mumax-delta * mumax, num_phen)
            np.fill_diagonal(fit_mat[ind_env], bad_mus)
            fit_mat[ind_env, ind_env, ind_env] = mumax

        mean_mu_max = sum(mumax * np.array(env_seq[1])) / total_time
        ...
        if OPT_SIGMOID_SWITCHMAXMIN:
            ...
            def fun_sigmoid_switching(x):
                """x contains as entries: beta, musens"""
                if verbose_optimization:
        ...
        if PLOT_SINGLE_SIMU:
            plot_single_simulation(fit_mat, env_seq, res_const.x, res_lin.x, res_exp.x, res_sigm.x, mumax, mumin, x0, num_env,
        ...
        results_df = results_df.append(
            {'simulation': simulation, 'num_env': num_env, 'num_phen': num_phen, 'mean_duration': mean_duration,
             'total_time': total_time, 'mumax': mumax, 'mean_mu_max': mean_mu_max, 'delta': delta,
    ...
    """Store results"""
    results_df_tidy.to_csv(os.path.join(workingdir, "results", "kl_parameterscan_sparse.csv"), index=False, header=True)
else:
    results_df_tidy = pd.read_csv(os.path.join(workingdir, "results", "kl_parameterscan_sparse.csv"))
...
plt.savefig(os.path.join(workingdir, "results", "kl_parameterscan_numenv.png"))

plt.figure(2)
sns.scatterplot(data=results_df_tidy, x='mean_duration', y='Gadv', hue='mudep_type_Gadv', alpha=0.5, s=scattersize)
plt.savefig(os.path.join(workingdir, "results", "kl_parameterscan_envduration.png"))

plt.figure(3)
sns.scatterplot(data=results_df_tidy, x='delta', y='Gadv', hue='mudep_type_Gadv', alpha=0.5,s=scattersize)
plt.savefig(os.path.join(workingdir, "results", "kl_parameterscan_growthrateseparation.png"))

plt.figure(4)
sns.scatterplot(data=results_df_tidy, x='simulation', y='Gadv', hue='mudep_type_Gadv', alpha=0.5,s=scattersize)
plt.savefig(os.path.join(workingdir, "results", "kl_parameterscan_simulation.png"))

plt.figure(5)
sns.scatterplot(data=results_df_tidy, x='G_const', y='Gadv', hue='mudep_type_Gadv', alpha=0.5, s=scattersize)
plt.savefig(os.path.join(workingdir, "results", "kl_parameterscan_G_const.png"))

g = sns.FacetGrid(data=results_df_tidy, col="mudep_type_Gadv", hue='mudep_type_Gadv', legend_out=True) #, height=4, aspect=.5
g.map(sns.scatterplot, "G_const", "Gadv", s=scattersize)
g.add_legend()
plt.savefig(os.path.join(workingdir, "results", "kl_parameterscan_panel.png"))
plt.savefig(os.path.join(workingdir, "results", "kl_parameterscan_panel.svg"))

g = sns.FacetGrid(data=results_df_tidy, col="mudep_type_Gadv", hue='delta', legend_out=True) #, height=4, aspect=.5
g.map(sns.scatterplot, "G_const", "Gadv", s=scattersize)
g.add_legend()
